...
class GetNorthComingTramsTest {
    ...
    private val routeRepository = mockk<RouteRepository>(){
        every { getNorthComingTrams() } returns Single.just(dummyRoutes)
    }

    @Test
    fun `should get list of route with Single decoration`() {
        val getNorthComingTramsUseCase = GetNorthComingTrams(
            routeRepository,
            TestSchedulerProvider()
        )
        val testObserver = getNorthComingTramsUseCase.get().test()
