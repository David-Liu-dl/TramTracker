...
class GetSouthComingTramsTest {
    ...
    private val routeRepository = mockk<RouteRepository>() {
        every { getSouthComingTrams() } returns Single.just(dummyRoutes)
    }

    @Test
    fun `should get list of route with Single decoration`() {
        val getSouthComingTramsUseCase = GetSouthComingTrams(
            routeRepository,
            TestSchedulerProvider()
        )
        val testObserver = getSouthComingTramsUseCase.get().test()
